...
public class BoardDAO {
   ...
   String sql_insert="insert into board(mid,msg) values (?,?)"; // 로그인에 성공한 경우에만 C 진행
   ...
   String sql_selectAll="select * from board where bid<= ? order by bid desc"; // "더보기": pagination(페이지네이션)
   
   String sql_insertR="insert into reply(bid,mid,rmsg) values(?,?,?)";
   ...
   public boolean insertR(ReplyVO vo) {
	      ...
	      try {
	        ...
	        // System.out.println(vo.getBid()+vo.getMid()+vo.getRmsg());
